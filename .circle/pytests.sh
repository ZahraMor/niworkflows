...
if [ "${CIRCLE_NODE_TOTAL:-1}" == "1" ]; then
    docker run -ti --env SAVE_CIRCLE_ARTIFACTS="/scratch" --env CIRCLE_NPROCS=${CIRCLE_NPROCS:-4} -v ${SCRATCH}/py3:/scratch -w /scratch --entrypoint=/usr/local/miniconda/bin/py.test niworkflows:py3 /root/niworkflows -n ${CIRCLE_NPROCS:-4} -v --junit-xml=/scratch/pytest.xml && \
    docker run -ti --env SAVE_CIRCLE_ARTIFACTS="/scratch" --env CIRCLE_NPROCS=${CIRCLE_NPROCS:-4} -v ${SCRATCH}/py2:/scratch -w /scratch --entrypoint=/usr/local/miniconda/bin/py.test niworkflows:py2 /root/niworkflows -n ${CIRCLE_NPROCS:-4} -v --junit-xml=/scratch/pytest.xml
    code=$(( $code + $? ))
else
